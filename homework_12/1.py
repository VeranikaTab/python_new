...
objects = ['История', 'Химия', 'Литература']
file = 'homework_12/objects.csv'
...
class Validate:
    ...
    def validate(self, value):
        for param in self.predicate:
            if not param(value):
                raise ValueError('ФИО должно состоять только из букв, первые заглавные!')
    ...
    def info(self):
        ...
        if self._upgrade:
            for key, value in self._journal.items():
                ...
                print(f"Средний балл тестов по предмету {key} :"
                      f" {round(sum(map(int, value['tests'])) / len(value['tests']))}")
...
if __name__ == '__main__':
    stud_1 = Student('Иванов', 'Степан', 'Михайлович')
    print(stud_1)
    stud_1.fill_journal()
    stud_1.info()
